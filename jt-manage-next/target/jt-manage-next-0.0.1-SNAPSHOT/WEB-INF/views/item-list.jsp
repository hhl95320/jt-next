        ...
        handler:function(){
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
        			...
        			// 加载商品描述
        			//_data = SysResult.ok(itemDesc) restful--风格传参数
        			//$.getJSON('/item/query/item/desc?id='+data.id,function(_data){
        			$.getJSON('/item/query/item/desc/'+data.id,function(_data){
        				if(_data.status == 200){
        					//UM.getEditor('itemeEditDescEditor').setContent(_data.data.itemDesc, false);
        					itemEditEditor.html(_data.data==null?"":_data.data.itemDesc);
        				}
        			});
        			...
        			$.getJSON('/item/param/item/query/'+data.id,function(_data){
        				if(_data && _data.status == 200 && _data.data && _data.data.paramData){
        					...
        					$("#itemeEditForm [name=itemParams]").val(_data.data==null?"":_data.data.paramData);
