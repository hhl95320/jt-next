...
//	}
	...
	public Object cacheRedisAround( ProceedingJoinPoint jp,CacheFind cacheFind) throws Throwable {
		...
		try {
			...
			Class returnType=(Class)map.get("returnType");

			if(jedis.exists(key)) {
				System.out.println("---redis缓存----"+key);
				long s=System.currentTimeMillis();
				object=ObjectMapperUtil.toObject(jedis.get(key), returnType);
				long e=System.currentTimeMillis();
				log.info("redis执行time:{}",e-s);
				return object;
			}
			else {
				long s=System.currentTimeMillis();
			      object=jp.proceed();
					long e=System.currentTimeMillis();
					log.info("数据库执行time:{}",e-s);
			  	System.out.println("---SQL查询----");
				    if(second==-1)//没有设置过期时间
				    	  jedis.set(key, ObjectMapperUtil.toJson(object));
				    else //设置过期时间
